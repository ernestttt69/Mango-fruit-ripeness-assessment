...
# Store each run separately so previous models and results are preserved.
OUTPUT_DIR = Path(__file__).resolve().parent / "runs" / (
    "clahe_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f")
)
OUTPUT_DIR.mkdir(parents=True, exist_ok=False)
_log_file = (OUTPUT_DIR / "training.log").open("w", encoding="utf-8", buffering=1)
...
sys.stdout = _Tee(_original_stdout)
sys.stderr = _Tee(_original_stderr)
...
if os.name == "nt":
    _awake = ctypes.windll.kernel32.SetThreadExecutionState(0x80000001)
    print("Automatic sleep prevention:", bool(_awake))
print("Device: CPU (OpenCV preprocessing and scikit-learn SVM)")
    ...
    for img_path in tqdm(
        ...
        for obj in root.findall(
            ...
            if display_count < 5:
                ...
                plt.tight_layout()
                save_figure(f"preview_{display_count + 1:02d}.png")
...
joblib.dump(svm_model, OUTPUT_DIR / "mango_svm_model.pkl")
joblib.dump(scaler, OUTPUT_DIR / "mango_scaler.pkl")
